...
import { Alert, AlertDetails } from './AlertDetails';
...
function isResolvedToday(resolvedAt?: string): boolean {
...
export function Alerts() {
  ...
  const resolvedTodayCount = alerts.filter((alert) => isResolvedToday(alert.resolvedAt)).length;

  const resolveAlert = (alertId: number) => {
    const resolvedAt = new Date().toISOString();
    setAlerts((prev) =>
      prev.map((alert) =>
        alert.id === alertId ? { ...alert, status: 'resolved', resolvedAt } : alert,
      ),
    );
    setSelectedAlert((prev) =>
      prev?.id === alertId ? { ...prev, status: 'resolved', resolvedAt } : prev,
    );
  };

  const unresolveAlert = (alertId: number) => {
    setAlerts((prev) =>
      prev.map((alert) =>
        alert.id === alertId ? { ...alert, status: 'active', resolvedAt: undefined } : alert,
      ),
    );
    setSelectedAlert((prev) =>
      prev?.id === alertId ? { ...prev, status: 'active', resolvedAt: undefined } : prev,
    );
  };

  const openAlertDrawer = (alert: Alert) => {
  ...
  const selectedAlertFromState = selectedAlert
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 lg:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-900 text-xl lg:text-2xl">{criticalCount}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4 lg:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-900 text-xl lg:text-2xl">{warningCount}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4 lg:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-900 text-xl lg:text-2xl">{resolvedTodayCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200">
          {activeAlerts.length === 0 ? (
            <div className="p-6 text-center text-sm text-gray-600">No active alerts</div>
          ) : (
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="text-xs px-2 lg:px-3 py-1 bg-white border border-gray-300 rounded hover:bg-gray-50 transition-colors"
                          >
                            View
                          </button>
                          <button
                            ...
                            className="text-xs px-2 lg:px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors"
                          >
                            Resolve
                          </button>
                        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200">
          {resolvedAlerts.length === 0 ? (
            <div className="p-6 text-center text-sm text-gray-600">No recently resolved alerts</div>
          ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <button
                          ...
                          className="text-xs px-2 lg:px-3 py-1 bg-white border border-gray-300 text-gray-700 rounded hover:bg-gray-50 transition-colors"
                        >
                          Unresolve
                        </button>
                      </div>
      ...
      {selectedAlertFromState && (
        <AlertDetails
          alert={selectedAlertFromState}
          onClose={closeAlertDrawer}
          onResolve={
            selectedAlertFromState.status === 'active' ? resolveAlert : undefined
          }
        />
      )}
    </div>
